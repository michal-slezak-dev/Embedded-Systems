...
unsigned char xdata Left _at_ 0xFD00;
unsigned char xdata Right _at_ 0xFE00;
...
volatile signed char data animationIndx = 26;
unsigned char isFirstClick = 0, timeTwoClicks;

unsigned char j = FROM;
...
void blank_bcd(unsigned char bcd_side) {
	if (bcd_side == 1) 			segmentLeft = 0x00;
	else if (bcd_side == 2) segmentRight = 0x00;
}
...
void displayCount(unsigned int n, int base) {
				if(n < base) {
				displayText('0', 1);
...
void ISR_INT() interrupt 0
{
    ...
    if (P3_2 == 0 && isPressed == 0) {
			...
			if(programState == STATE_WAIT_FOR_CLICK) // czekam na druga akcje uzytkownika
			{
			...
			// podwojne wcisniecie
			else if(programState == STATE_WAIT_FOR_DOUBLE_CLICK) // jestem w stanie 2 i czekam na trzecia akcje uzytkownika
			{
				if(!isFirstClick)
				{
					isFirstClick = 1;
...
{
	if(isFirstClick) timeTwoClicks++;
	
	if(animationNum == 1) // przewijanie tekstu w prawo automatycznie, jestem w stanie 1
	{
	...
	else if(animationNum == 2) // jestem w stanie 2
	{
	...
	else if(animationNum == 3) // jestem w stanie 3
	{		
		// rosnaco
