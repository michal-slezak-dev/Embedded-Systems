...
/* char textErr[] = "ER";
char textErrSize = sizeof(textErr);
	
char textOk[] = "OK";
char textOkSize = sizeof(textOk); */
  ...
  if(RI == 1)
   {
     RI = 0;
		 receivedByte = SBUF;
		 sleep();
     if(receivedByte == '0') {correct = 1;}
     else if(receivedByte == '1') {incorrect = 1;}
     //IE1 = 1;
   }
		//RI = 1;
		//ringing = 1;
...
{	
	if(ringing == 1)
	{
		...
		if (i >= textSize) {i = 0; ringing = 0; blank_bcd(1);}
	}else
	{
		//blank_bcd(1);
		if(correct)
